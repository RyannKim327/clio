...
export default function Modal({ className, closeModal, show, children }: modal) {
  return (
    <div
      ...
      <div
        className={`${className ?? "max-w-[calc(75%-0.5rem)] md:max-w-[calc(50%-1rem)]"} max-h-[calc(50%-0.5rem)] flex bg-secondary-bg p-3 rounded pointer-events-auto overflow-y-auto`}>
        {children}
      </div>
    </div>
  )
