...
export default function Book({
  ...
  return (
    <div className="relative aspect-9/16 border border-solid border-border rounded group overflow-hidden">
      ...
      <Link
        to={`/read?id=${id}`}
        className="flex flex-col p-2 max-h-1/3 absolute bottom-0 z-10 bg-linear-to-b from-bg/25 to-bg w-full opacity-0 transition-all delay-75 group-hover:opacity-100">
        <span>{title}</span>
